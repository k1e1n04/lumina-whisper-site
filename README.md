...
- Lumina Whisper は現在 Direct 配布（DMG）のみです
- Mac App Store 配信は終了しています
...
`public/appcast.xml` は Lumina Whisper 本体の自動アップデート（Sparkle）で参照されます。  
公開URLは `https://lumina-whisper.com/appcast.xml` です。

更新コマンド:

```bash
npm run appcast:update -- \
  --version 3 \
  --short-version 0.3.0 \
  --url "https://github.com/k1e1n04/lumina-whisper-site/releases/latest/download/LuminaWhisper.dmg" \
  --length 123456789 \
...
- `--version` は `CFBundleVersion`（整数）
- `--short-version` は `CFBundleShortVersionString`（例: `0.3.0`）
- `--ed-signature` は `lumina-whisper/scripts/sign-update.sh --file <DMG_PATH>` で取得可能
